                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <button
                          onClick={() => {
                            ...
                            handleUpdateUser(trialUser, "🎉 7-Day VIP Free Trial Activated! Sponsor banner hidden.");
                            setIsHamburgerOpen(false);
                          }}
                          className="p-3 rounded-xl bg-primary-500/20 hover:bg-primary-500/30 border border-primary-500/40 text-primary-300 text-left cursor-pointer transition space-y-1"
                        >
                          <div className="flex items-center gap-1.5 font-extrabold text-xs">
                            <Clock className="w-3.5 h-3.5 text-primary-400" />
                            <span>7-Day VIP Trial</span>
                          </div>
                          <p className="text-[10px] text-zinc-400">Test drive VIP perks (Hides ads)</p>
                        </button>
                        ...
                        <button
                          onClick={() => {
                            ...
                            handleUpdateUser(adFreeUser, "⚡ Permanent VIP Status Unlocked! Sponsor banner hidden.");
                            setIsHamburgerOpen(false);
                          }}
                          className="p-3 rounded-xl bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-500/40 text-emerald-300 text-left cursor-pointer transition space-y-1"
                        >
                          <div className="flex items-center gap-1.5 font-extrabold text-xs">
                            <Crown className="w-3.5 h-3.5 text-emerald-400" />
                            <span>Buy VIP Pass (₱70)</span>
                          </div>
                          <p className="text-[10px] text-zinc-400">Permanent VIP & No Ads</p>
                        </button>
                      </div>

                      {/* Expire / Reset VIP button to test banner reappearing */}
                      {hasAdFree ? (
                        <button
                          onClick={() => {
                            const expiredUser = {
                              ...
                              vipTierName: 'Basic Tier',
                              trialEndsAt: new Date(Date.now() - 1000).toISOString() // Expired timestamp
                            };
                            handleUpdateUser(expiredUser, "⚠️ VIP Pass Expired! Sponsor banner is now visible again.");
                            setIsHamburgerOpen(false);
                          }}
                          className="w-full py-2 px-3 rounded-xl bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-300 font-bold text-xs flex items-center justify-center gap-2 cursor-pointer transition mt-1"
                        >
                          <Ban className="w-3.5 h-3.5 text-red-400" />
                          <span>Simulate VIP Expired (Shows Banner)</span>
                        </button>
                      ) : (
                        <div className="p-2.5 rounded-xl bg-[color-mix(in_srgb,var(--card-bg)_80%,white)] border border-zinc-800 text-[11px] text-zinc-400 flex items-center gap-2">
                          <Crown className="w-4 h-4 text-primary-400 shrink-0" />
                          <span>VIP Pass Expired/Inactive — Sponsor banner is shown below.</span>
                        </div>
                      )}
                    </div>
                  )}
